...
export function useFlowPipes() {
  // 存储所有创建的实体，方便后续管理
  const pipeEntities = ref<any[]>([])
  const isGenerating = ref(false)
  const generationError = ref<Error | null>(null)

  // 生成流动管线和管点
  const generateFlowPipes = (viewer: any) => {
  ...
  // 添加单条管线
  const addSinglePipe = (viewer: any, pipeData: PipeData) => {
    try {
      // 1. 添加基础管线（半透明背景线）
      ...
      pipeEntities.value.push(baseLineEntity)
      ...
      pipeEntities.value.push(glowLineEntity)
  ...
  // 添加流动点效果
  const addFlowPoints = (viewer: any, pipeData: PipeData) => {
    ...
    for (let i = 0; i < pointCount; i++) {
      ...
      pipeEntities.value.push(pointEntity)
    }
  }

  // 添加管点（起点、终点和中间节点）
  const addPipeNodes = (viewer: any, pipeData: PipeData) => {
    const positions = pipeData.positions
    ...
    pipeEntities.value.push(startNode)
    ...
    pipeEntities.value.push(endNode)
    ...
    for (let i = 1; i < positions.length - 1; i++) {
      ...
      pipeEntities.value.push(middleNode)
    }
  }

  // 清除所有管线实体
  const clearPipeEntities = (viewer: any) => {
    pipeEntities.value.forEach((entity) => {
      try {
    ...
    pipeEntities.value.length = 0
  }

  // 相机视角动画移动
  const flyToPipeArea = (viewer: any, duration: number = 4) => {
    viewer.camera.flyTo({
